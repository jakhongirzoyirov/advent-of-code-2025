...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String[] db = split[1].split("\n");
        List<long[]> intervals = new ArrayList<>();
        for (int i = 0; i < ranges.length; i++) {
            String[] split1 = ranges[i].split("-");
            intervals.add(new long[]{Long.parseLong(split1[0]), Long.parseLong(split1[1])});
        }
        System.out.println(day2(intervals));
    }

    private static long day2(List<long[]> intervals) {
        ...
        for (int i = 0; i < merged.size(); i++) {
            count += merged.get(i)[1] - merged.get(i)[0] + 1;
        }
    ...
    private static int day1(String[] ranges, String[] db) {
        int count = 0;
        for (int i = 0; i < db.length; i++) {
            long num = Long.parseLong(db[i]);
            for (int j = 0; j < ranges.length; j++) {
                String[] split = ranges[j].split("-");
                if (Long.parseLong(split[0]) <= num && num <= Long.parseLong(split[1])) {
                    count++;
                    break;
                }
            }
    ...
    private static String getInput() {
        return "213205509444883-217326119178383\n" +
